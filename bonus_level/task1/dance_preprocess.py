...
Example:
    python preprocess_template.py
    (Then follow the interactive prompt to enter video path)
"""
...
def parse_args():
    ...
    # 将 video 改为 nargs="?"（可选参数），以便用户直接运行脚本时不会因缺少参数报错
    parser.add_argument("video", type=Path, nargs="?", default=None, help="Path to the template video")
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=Path(__file__).resolve().parent / "dance_pose_data",
        help="Parent output directory (default: dance_pose_data)",
    )
    parser.add_argument(
        "--model",
...
def preprocess(args):
    # 如果命令行没有提供视频路径，则在终端进行英文提示并获取输入
    if args.video is None:
        try:
            user_input = input("Please enter the path to the dance video file: ").strip()
            if not user_input:
                raise ValueError("Video path cannot be empty.")
            # 兼容带有双引号或单引号的文件路径拖入
            user_input = user_input.strip("'\"")
            args.video = Path(user_input)
    ...
    print("\n[Preprocessing started... Please wait]")
    ...
    print(f"\nSelection mode: {selection_method}")
