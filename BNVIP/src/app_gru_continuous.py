"""Webcam continuous multi-sign recognition with GRU sliding windows.

Usage:
    python src/app_gru_continuous.py

Same UX as the DTW/fast continuous apps, but classifies sliding windows with
the existing sign_gru.pt model.
...
from gru_match import recognize_stream
...
def draw_ui(frame, recording, n_frames, status, words, user, reply, llm_up):
    ...
    cv2.putText(
        frame,
        "GRU window | space=start/stop  r=send  c=clear  n=new  q=quit",
        (10, h - 10),
        cv2.FONT_HERSHEY_SIMPLEX,
        0.45,
        (180, 180, 180),
...
def main():
    ...
    words, user, reply = [], "", ""
    status = "GRU window mode: space = start continuous signing"
    recording = False
    buf = []
    rec_started = None
    ...
    while True:
        ...
        cv2.imshow("BNVIP - continuous GRU signs", frame)
