"""Webcam continuous multi-sign recognition (fast resample similarity).

Usage:
    python src/app_fast_continuous.py

Same UX as app_dtw_continuous.py, but uses normalize + resample + cosine
similarity instead of DTW. Existing DTW files are left unchanged.
...
from fast_match import load_word_templates, recognize_stream
...
def draw_ui(frame, recording, n_frames, status, words, user, reply, llm_up):
    ...
    cv2.putText(
        frame,
        "FAST mode | space=start/stop  r=send  c=clear  n=new  q=quit",
        (10, h - 10),
        cv2.FONT_HERSHEY_SIMPLEX,
        0.45,
        (180, 180, 180),
...
def main():
    ...
    words, user, reply = [], "", ""
    status = "FAST mode: space = start continuous signing"
    recording = False
    buf = []
    rec_started = None
    ...
    while True:
        ...
        cv2.imshow("BNVIP - continuous FAST signs", frame)
