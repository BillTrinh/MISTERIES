"""Stronger fast similarity matching (dance-score inspired, no DTW).
...
def motion_similarity(seq_a: np.ndarray, seq_b: np.ndarray) -> float:
    """Dance-like motion score on fixed-length keypoint sequences."""
    ...
    if int(np.sum(moving)) < 2:
        # Nearly static template: fall back to whole-delta cosine.
...
def combined_similarity(window_fixed: np.ndarray, template_fixed: np.ndarray) -> dict:
    """Pose + motion blend, both sequences already (T, 126)."""
...
def _best_template_score(
    ...
    *,
    ...
    if already_fixed:
        ...
        # Stream was normalize_sequence()'d once; only resample windows.
...
def scan_stream_for_word(
    ...
    for scale in scales:
        ...
        for start in range(0, len(stream) - win + 1, hop):
            ...
            # Only spend lag search on near-miss / good windows.
