"""Webcam continuous multi-sign recognition with FastDTW.

Usage:
    python src/app_fastdtw_continuous.py

Same UX as app_dtw_continuous.py, but uses multiresolution FastDTW
instead of band-limited exact DTW. Leaves dtw_match.py unchanged.
...
from fastdtw_match import load_word_templates, recognize_stream
...
def draw_ui(frame, recording, n_frames, status, words, user, reply, llm_up):
    ...
    cv2.putText(
        frame,
        "FastDTW | space=start/stop  r=send  c=clear  n=new  q=quit",
        (10, h - 10),
        cv2.FONT_HERSHEY_SIMPLEX,
        0.45,
        (180, 180, 180),
...
def main():
    print("Loading FastDTW word templates ...")
    ...
    while True:
        ...
        cv2.imshow("BNVIP - continuous FastDTW signs", frame)
