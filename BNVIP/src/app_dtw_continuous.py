"""Webcam continuous multi-sign recognition with DTW.

Usage:
    python src/app_dtw_continuous.py

Unlike app_dynamic.py (one word per space press), this records a longer
clip and detects MULTIPLE word-signs inside it.
...
def draw_ui(frame, recording, n_frames, status, words, user, reply, llm_up):
    ...
    cv2.putText(
        frame,
        "space=start/stop  r=send  c=clear  n=new  q=quit",
        (10, h - 10),
        cv2.FONT_HERSHEY_SIMPLEX,
        0.45,
        (180, 180, 180),
...
def main():
    print("Loading DTW word templates ...")
    ...
    while True:
        ...
        cv2.imshow("BNVIP - continuous DTW signs", frame)
